...
# Ensure the src directory is on the path to import constants
...
def _generate_paginated_call_block(command_name_camel: str, original_result_model: str, paginated_result_model: str,
    ...
    return dedent(f'''
        ...
        if time.time() - timestamp > PAGINATION_CACHE_LIFETIME_SECONDS:
...
def _generate_tool_function_code(command: dict, valid_model_names: set[str]) -> str:
    ...
    is_paginated = command_name_camel in constants.PAGINATED_COMMANDS
    ...
    if is_paginated:
        list_attribute_name = constants.PAGINATED_COMMANDS[command_name_camel]
        ...
        docstring_addendum = constants.PAGINATED_DOCSTRING_ADDENDUM
    ...
    docstring = f'''"""\n    {command["description"]}{docstring_addendum}\n\n    To find a valid 'port' number, use the 'list_active_archicads' tool.\n    """'''
...
def generate_tool_files(grouped_commands: dict[str, list[dict]], out_dir: Path, valid_model_names: set[str]):
    for group_name, commands in grouped_commands.items():
        ...
        is_any_paginated = any(cmd["name"] in constants.PAGINATED_COMMANDS for cmd in commands)

        imports_block = _generate_imports_for_group(commands, valid_model_names)

        common_imports = [
            constants.GENERATION_FILE_HEADER,
            "import logging",
        ...
        if is_any_paginated:
            common_imports.extend([
                ...
                "from tapir_archicad_mcp.pagination import handle_paginated_request, PAGINATION_CACHE",
                "from tapir_archicad_mcp.constants import PAGINATION_CACHE_LIFETIME_SECONDS"
            ])
        if imports_block:
...
def main():
    ...
    command_details_all = fetch_json_data(constants.COMMAND_DETAILS_URL, "command details")
    valid_model_names_list = fetch_json_data(constants.COMMAND_MODEL_NAMES_URL, "valid model names")

    if not command_details_all or not valid_model_names_list:
        log.critical("Could not fetch necessary data. Aborting generation.")
        return

    log.info(f"Excluding {len(constants.COMMANDS_TO_EXCLUDE)} commands from generation.")
    command_details_filtered = [
        cmd for cmd in command_details_all if cmd["name"] not in constants.COMMANDS_TO_EXCLUDE
    ]
