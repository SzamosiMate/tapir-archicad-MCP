...
COMMANDS_TO_EXCLUDE = {
    "GetProjectInfo",  # Redundant: Handled by our custom 'list_active_archicads' tool.
    "GetArchicadLocation",  # Redundant: Also discoverable via the custom tool.
    "QuitArchicad",  # Potentially dangerous for an AI to call without specific safeguards.
    "GenerateDocumentation", # Internal command for Tapir development
    "GetRevisionIssues", #Tapir schema has inconsistent DocumentRevision definition
    "SetPropertyValuesOfAttributes", #Tapir schema is missing AttributePropertyValues definition
    "CreateZones", #Tapir schema has inconsistent Geometry definition
}
...
def _generate_paginated_model_code(original_result_model: str, paginated_result_model: str,
    ...
    """Generates the Pydantic model class for a paginated response."""
    return dedent(f'''
...
def _generate_standard_call_block(command_name_camel: str, params_model: str, result_model: str, has_params: bool,
                                  has_result: bool) -> str:
    """Generates the inner `try` block for a standard, non-paginated API call."""
...
def _generate_paginated_call_block(command_name_camel: str, params_model: str, original_result_model: str,
                                   paginated_result_model: str, list_attribute_name: str) -> str:
    """Generates the inner `try` block for a paginated API call."""
    params_for_api_call = "params.model_dump(mode='json')"
    return dedent(f'''
        cache_key = f"{{port}}:{command_name_camel}:{{params.model_dump_json() if params else ''}}"
...
def _generate_tool_function_code(command: dict, valid_model_names: set[str]) -> str:
    """Generates the complete code for a single tool, handling both standard and paginated cases."""
    ...
    # --- Determine signature, return types, and call block ---
    ...
    if is_paginated:
        ...
        call_block = _generate_paginated_call_block(command_name_camel, params_model, result_model,
                                                    paginated_result_model, list_attribute_name)
    else:
        return_annotation = result_model if has_result else "None"
        call_block = _generate_standard_call_block(command_name_camel, params_model, result_model, has_params,
                                                   has_result)

    # --- Assemble the final function code ---
