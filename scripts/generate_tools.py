...
PAGINATED_DOCSTRING_ADDENDUM = (
    ...
    "again with the same arguments and page_token set to that token to get the next page of results."
)
...
def _generate_tool_function_code(command: dict, valid_model_names: set[str], config: ApiSourceConfig) -> str:
    ...
    docstring = format_docstring(
        command["description"],
        addendum=PAGINATED_DOCSTRING_ADDENDUM if is_paginated else "",
    )
    dispatch_desc = format_string_literal(command["description"])

    decorator = ""
    if REGISTER_AS_MCP_TOOLS:
        mcp_desc = format_string_literal(command["description"])
...
'''
    ...
    registration_call = f"""register_tool_for_dispatch(
    {command_name_snake},
    name="{tool_name}",
    title="{command_name_camel}",
...
def run_generation_for_source(config: ApiSourceConfig) -> Set[str]:
    ...
    run_ruff_format(config.output_dir)
