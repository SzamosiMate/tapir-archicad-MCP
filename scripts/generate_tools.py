...
def _generate_tool_function_code(command: dict, valid_model_names: set[str]) -> str:
    ...
    short_group_name = GROUP_NAME_MAPPING.get(command["group"])
    if not short_group_name:
        log.warning(
            f"No short name mapping found for group '{command['group']}'. Skipping tool '{command_name_camel}'.")
        return ""
    ...
    docstring = indent(DOCSTRING_TEMPLATE.format(description=command["description"]), "    ")
...
def generate_tool_files(grouped_commands: dict[str, list[dict]], out_dir: Path, valid_model_names: set[str]):
    for group_name, commands in grouped_commands.items():
        ...
        common_imports = [
            FILE_HEADER,
            "", "import logging", "from pydantic import ValidationError",
        ...
        common_imports.append("log = logging.getLogger()\n")
        ...
        with open(file_path, "w", encoding="utf-8") as f:
            f.write("\n".join(file_content))  # Use double newline for better spacing between functions
